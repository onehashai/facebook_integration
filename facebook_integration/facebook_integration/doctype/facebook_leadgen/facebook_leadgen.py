	...
	def after_insert(self):
		if self.page_id and self.leadgen_id and self.status:
			if frappe.get_value("Facebook Forms", self.form_id, "enabled") == 0:
				self.status = "Failed"
				self.save()
				frappe.log_error('Facebook Form Disabled', "Facebook Form is disabled for facebook leadgen: "+self.name)
			else:
				result = frappe.get_value("Facebook Pages", filters={"page_id": self.page_id}, fieldname=["parent", "page_access_token"])
				client, page_access_token=result
				if page_access_token:
					...
					self.status = "Failed"
					self.save()
					frappe.log_error('No Facebook Page Access Token', "Page access token was not found for facebook leadgen: "+self.name)
			frappe.db.commit()
...
def lead_insertion(doc, client, page_access_token):
	...
	try:
		...
		doc.status = "Failed"
		doc.save()
		frappe.log_error("Error Facebook Leadgen", "Error occured while fetching facebook leadgen data for leadgen id: {} ".format(doc.leadgen_id) + frappe.get_traceback())
		return
	...
	if doc.form_id:
		try:
			...
			doc.status = "Failed"
			doc.save()
			frappe.log_error("Error: Facebook Form Not Found", "Facebook Form {} not found for leadgen id: {} \n".format(doc.form_id, doc.leadgen_id) + frappe.get_traceback())
			return
	...
	if lead_data and lead_data.get("error"):
		doc.status = "Failed"
		doc.save()
		frappe.log_error("Error Facebook Leadgen", "Facebook leadgen data fetch produced this error: "+str(lead_data.get("error")))
		return
	else:
		...
		try:
			...
			frappe.local.initialised = False
			frappe.connect(site=master_site)
			frappe.log_error("Error Saving Facebook Lead", "Error occured while saving lead into client: {} :".format(client_domain)+frappe.get_traceback())
		if res.get("name"):
			doc.status = "Success"
			doc.save()
		else:
			doc.status = "Failed"
			doc.save()
			frappe.log_error("Facebook Lead Creation Failed", "Saving lead on client's system: {} failed for leadgen id: {}".format(client_domain, doc.leadgen_id))
